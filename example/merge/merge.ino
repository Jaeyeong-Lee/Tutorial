...
// SerialUSB.println(s) .....????????????
//#define error(s) sd_fat.errorHalt(F(s))
#define error(s) SerialUSB.println(s);
...
void setup() {
  ...
  if ( !sd_fat.begin(chipSelect, SPI_HALF_SPEED) )
    error("sd card initialize failed");
  ...
  if ( menu_redraw_required != 0 ) {
    ...
    switch (mymenu) {
      ...
      default:
        ...
        // 160803 simple implementation.
        if (mymenu == LIVEPLAY_MENU) menu_LivePlay();

        // why occur compile error??
        //      if(mymenu == DOWNLOAD_MENU) menu_DownLoad();
